...
{
#if 0
    HSD_TExp *diff, *spec, *ext;
    HSD_TExp *texp, *texp_00, *exp, *cnst;
    HSD_TObj *tobj, *toon = NULL;

    HSD_CheckAssert("MObjMakeTExp: list == NULL", list != NULL);
    ...
    *list = NULL;
    ...
    if ((mobj->rendermode & RENDER_DIFFUSE_VTX) == 0) {
    ...
    for (tobj = tobj_top; tobj != NULL; tobj = tobj->next) {
        if ((tobj_lightmap(tobj) & (TEX_LIGHTMAP_DIFFUSE | TEX_LIGHTMAP_AMBIENT)) && tobj->id != GX_TEXMAP_NULL) {
            HSD_TOBJ_METHOD(tobj)->make_texp(tobj, TEX_LIGHTMAP_DIFFUSE | TEX_LIGHTMAP_AMBIENT, 0, &diff, &exp, list);
        }
    }

    if ((mobj->rendermode & RENDER_DIFFUSE) != 0) {
        texp = HSD_TExpTev(list);

        if (toon == NULL) {
            HSD_TExpOrder(texp, NULL, GX_COLOR0A0);
            HSD_TExpColorOp(texp, GX_TEV_ADD, GX_TB_ZERO, GX_CS_SCALE_1, GX_ENABLE);
            HSD_TExpColorIn(texp, HSD_TE_0, HSD_TEXP_ZERO,
                HSD_TE_0, HSD_TEXP_ZERO,
                HSD_TE_0, HSD_TEXP_ZERO,
                HSD_TE_RGB, diff);
        } else {
            HSD_TExpOrder(texp, toon, GX_COLOR0A0);
            HSD_TExpColorOp(texp, GX_TEV_ADD, GX_TB_ZERO, GX_CS_SCALE_1, GX_ENABLE);
            HSD_TExpColorIn(texp, HSD_TE_0, HSD_TEXP_ZERO,
                HSD_TE_RGB, diff,
        ...
        diff = texp;
        HSD_TExpAlphaOp(texp, GX_TEV_ADD, GX_TB_ZERO, GX_CS_SCALE_1, GX_ENABLE);
        HSD_TExpAlphaIn(texp, HSD_TE_0, HSD_TEXP_ZERO,
            HSD_TE_A, exp,
            HSD_TE_A, HSD_TEXP_RAS,
            HSD_TE_0, HSD_TEXP_ZERO);
        exp = texp;
    ...
    if (mobj->rendermode & RENDER_SPECULAR) {
        ...
        spec = HSD_TExpTev(list);
        HSD_TExpColorOp(spec, GX_TEV_ADD, GX_TB_ZERO, GX_CS_SCALE_1, GX_ENABLE);
        HSD_TExpColorIn(spec, HSD_TE_0, HSD_TEXP_ZERO,
            HSD_TE_0, HSD_TEXP_ZERO,
            HSD_TE_0, HSD_TEXP_ZERO,
            HSD_TE_RGB, cnst);
        ...
        for (tobj = tobj_top; tobj != NULL; tobj = tobj->next) {
            if ((tobj_lightmap(tobj) & TEX_LIGHTMAP_SPECULAR) && tobj->id != GX_TEXMAP_NULL) {
                HSD_TOBJ_METHOD(tobj)->make_texp(tobj, TEX_LIGHTMAP_SPECULAR, 0x50, &spec, &exp, list);
            }
        }

        texp = HSD_TExpTev(list);
        HSD_TExpOrder(texp, NULL, GX_COLOR1A1);
        HSD_TExpColorOp(texp, GX_TEV_ADD, GX_TB_ZERO, GX_CS_SCALE_1, GX_ENABLE);
        HSD_TExpColorIn(texp, HSD_TE_0, HSD_TEXP_ZERO,
            HSD_TE_RGB, spec,
            HSD_TE_RGB, HSD_TEXP_RAS,
        ...
        texp_00 = HSD_TExpTev(list);
        HSD_TExpColorOp(texp_00, GX_TEV_ADD, GX_TB_ZERO, GX_CS_SCALE_1, GX_ENABLE);
        HSD_TExpColorIn(texp_00, HSD_TE_RGB, spec,
            HSD_TE_0, HSD_TEXP_ZERO,
            HSD_TE_0, HSD_TEXP_ZERO,
            HSD_TE_RGB, diff);
        diff = texp_00;
    ...
    for (tobj = tobj_top; tobj != NULL; tobj = tobj->next) {
        if ((tobj_lightmap(tobj) & TEX_LIGHTMAP_EXT) && tobj->id != GX_TEXMAP_NULL) {
            HSD_TOBJ_METHOD(tobj)->make_texp(tobj, TEX_LIGHTMAP_EXT, 0x70, &ext, &exp, list);
        }
    }

    if (ext != exp || HSD_TExpGetType(ext) != HSD_TE_TEV || HSD_TExpGetType(exp) != HSD_TE_TEV) {
        texp_00 = HSD_TExpTev(list);
        HSD_TExpColorOp(texp_00, GX_TEV_ADD, GX_TB_ZERO, GX_CS_SCALE_1, GX_ENABLE);
        HSD_TExpColorIn(texp_00, HSD_TE_0, HSD_TEXP_ZERO,
            HSD_TE_0, HSD_TEXP_ZERO,
            HSD_TE_0, HSD_TEXP_ZERO,
            HSD_TE_RGB, ext);
        HSD_TExpAlphaOp(texp_00, GX_TEV_ADD, GX_TB_ZERO, GX_CS_SCALE_1, GX_ENABLE);
        HSD_TExpAlphaIn(texp_00, HSD_TE_0, HSD_TEXP_ZERO,
            HSD_TE_0, HSD_TEXP_ZERO,
            HSD_TE_0, HSD_TEXP_ZERO,
            HSD_TE_A, exp);
        ext = texp_00;
    ...
    *list = NULL;
    ...
    if ((mobj->rendermode & RENDER_DIFFUSE) != 0) {
        if ((alpha_mode & RENDER_ALPHA_VTX) != 0) {
            exp = HSD_TExpTev(list);
...
void HSD_MObjSetup(HSD_MObj* mobj, u32 rendermode)
{
    ...
    if ((rendermode & RENDER_SPECULAR) != 0) {
    ...
    if ((rendermode & RENDER_SHADOW) != 0 && tobj_shadows != NULL) {
        for (tail = &tobj; *tail != NULL; tail = &((*tail)->next)) {
        }
        *tail = tobj_shadows;
    }
    if ((rendermode & RENDER_TOON) != 0 && tobj_toon != NULL && tobj_toon->imagedesc != NULL) {
        tobj_toon->next = tobj;
        tobj = tobj_toon;
    }
    HSD_TObjSetup(tobj);
...
HSD_MObj* HSD_MObjAlloc(void)
{
    ...
    HSD_CheckAssert("HSD_MObjAlloc: Could not alloc mobj", mobj != NULL);
    return mobj;
}
...
HSD_Material* HSD_MaterialAlloc(void)
{
    ...
    HSD_CheckAssert("HSD_MaterialAlloc: Could not alloc mat", mat != NULL);
